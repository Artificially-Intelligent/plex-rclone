...
HEALTHCHECK --interval=5m --timeout=3s \
  CMD ! $DO_HEALTH_CHECK || [ "$(ls -A $MEDIA_MOUNT_CONTAINER_PATH)"  ] || [ "$(ls -A /plex/media/)" ]  || $(ps aux | grep -q "[r]clone sync")  || $(ps aux | grep -q "[t]ar -C") || ! echo "Health Check Failed" || ( $HEALTH_CHECK_KILL && echo "Stopping Container" && kill 1 ) || exit 1
